...
ll knapsack(int i, int w, vll &weights, vll &values) {
    ...
    int notTake = knapsack(i-1, w, weights, values);
    int take = INT_MIN;
    if (w >= weights[i]) take = values[i] + knapsack(i-1, w-weights[i], weights, values);
